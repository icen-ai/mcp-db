...
- `grants[]`:权限清单。`ops`: `read` → `GRANT SELECT`;`write` → +`INSERT/UPDATE/DELETE`;`ddl` 不做表级物化。`tables: "*"` 全部表;数组元素支持 * 通配模式,可出现在任意位置、可多个(如 ty_*、*_tmp、a_*_b),同步时按 schema 实际表集展开为精确 GRANT。
